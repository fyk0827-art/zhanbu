...
import { ageGroupApi, adminPaymentApi, adminQuestionApi, answerApi, adminSettingsApi, paymentApi, domainConfigApi } from "@/services/api";
import type { AdminQuestionDTO, CreateQuestionRequest, AgeGroup } from "@/types/api";
import { useAdminAuth } from "@/hooks/useAdminAuth";
import LanguageSwitcher from "@/components/LanguageSwitcher";

type Tab = "questions" | "ageGroups" | "answers" | "orders" | "settings" | "domains";
    ...
    <div className="min-h-screen bg-[#FFFDF5]">
      ...
      <div className="mx-auto max-w-[1400px] px-6 py-6">
        <div className="mb-6 flex gap-2 border-b border-[#E8E4DC]">
          {(["questions", "ageGroups", "answers", "orders", "settings", "domains"] as Tab[]).map((tab) => (
            <button
              key={tab}
              ...
              <div className="flex items-center gap-2 text-sm font-medium text-[#2D2A26]">
                ...
                {paypalConfigured
                  ? settings?.paymentMode === "live"
                    ? t("paymentModePaypalLive", "PayPal Live") + " ✓"
                    : t("paymentModePaypalSandbox", "PayPal Sandbox") + " (sandbox)"
                  : t("paymentModePaypalMissing", "PayPal is not configured")}
              </div>
              <p className="mt-2 text-xs leading-relaxed text-[#6B6560]">
                {settings?.paymentMode === "live"
                  ? t("paymentModeLiveHelp", "Real payments via PayPal Live. Before each test, set the price to a small amount (e.g. $0.01).")
                  : t("paymentModeHelp", "Sandbox payments via PayPal Sandbox. Use the test account for verification.")}
              </p>
